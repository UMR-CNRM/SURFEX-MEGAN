...
!
REAL, DIMENSION(:), ALLOCATABLE :: T24                   ! average temperature over the past 24h (K) 
REAL, DIMENSION(:), ALLOCATABLE :: PPFD24                ! average PAR over the past 24h         (umol/m2.s) 
...
REAL, DIMENSION(:), ALLOCATABLE :: ZTSOMME, ZPSOMME               
INTEGER :: ISERIES, ISIZE, JT, JP
REAL :: JDAY
!
! MPI variables
!
...
IF (.NOT.ALLOCATED(ZTSOMME)) ALLOCATE(ZTSOMME    (INI)) 
IF (.NOT.ALLOCATED(T24)) ALLOCATE(T24    (INI)) 
IF (.NOT.ALLOCATED(PPFD24)) ALLOCATE(PPFD24    (INI))
IF (.NOT.ALLOCATED(ZPSOMME)) ALLOCATE(ZPSOMME    (INI))
!
...
CALL INIT_SURF_LANDUSE_n(YSC%DTCO, YSC%DUO%LREAD_BUDGETC, YSC%U, YSC%UG,   &
                         ...
                         XSCA_ALB, XEMIS, XTSRAD, XTSURF, YSC%IM%MSF%XMEGAN_FIELDS, IYEAR, IMONTH,   &
...
!To calculate T24
    ...
    CALL COUPLING_SURF_ATM_n(YSC, CSURF_FILETYPE, 'E', ZTIMEC, XTSTEP_SURF, IYEAR, IMONTH, IDAY, ZTIME, &
                             ...
                             XPEW_B_COEF,XPET_A_COEF,XPEQ_A_COEF,XPET_B_COEF,XPEQ_B_COEF, YTEST, T24, XP_PPFD, PPFD24, ICOUNT      )

                    
!To calculate PPFD24
   IF (JP .EQ. IDAY) THEN
       ZPSOMME(:) = ZPSOMME(:) + XP_PPFD(:)
   ELSE
       PPFD24(:) = ZPSOMME(:)/288.
       JP = IDAY
       ZPSOMME(:) = 0.
