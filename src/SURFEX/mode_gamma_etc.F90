...
!-----------------------------------------------------------------------
SUBROUTINE GAMMA_S(PSOILM, PWWILT, PGAM_S)

IMPLICIT NONE
REAL, DIMENSION(:), INTENT(IN) :: PSOILM, PWWILT
REAL, DIMENSION(:), INTENT(OUT) :: PGAM_S

REAL, DIMENSION(SIZE(PSOILM)) :: ZTHETA1
REAL, PARAMETER :: ZDELTA_THETA = 0.04 !(=0.06 modified to compare with Paul's results)
INTEGER :: JJ
 
!DO JJ = 1,SIZE(PSOILM)
  
!  ZTHETA1(JJ) = PWWILT(JJ) + ZDELTA_THETA
  
!  IF (PSOILM(JJ) .GT. ZTHETA1(JJ)) THEN
!     PGAM_S(JJ) = 1.
!  ELSEIF (PSOILM(JJ) .GE. PWWILT(JJ) .AND. PSOILM(JJ) .LT. ZTHETA1(JJ)) THEN
!     PGAM_S(JJ) = ( PSOILM(JJ) - PSOILM(JJ))/ZDELTA_THETA
!  ELSEIF (PSOILM(JJ) .LT. PWWILT(JJ)) THEN
!     PGAM_S(JJ) = 0.
!  END IF
...
DO JJ = 1,SIZE(PSOILM)
  ...
  IF (PSOILM(JJ) .LT. PWWILT(JJ)) THEN
     PGAM_S(JJ) = 0.
  ELSEIF (PSOILM(JJ) .GE. PWWILT(JJ) .AND. PSOILM(JJ) .LT. ZTHETA1(JJ)) THEN
     PGAM_S(JJ) = ( PSOILM(JJ) - PWWILT(JJ) )/ZDELTA_THETA
  ELSE
     PGAM_S(JJ) = 1.
  END IF
