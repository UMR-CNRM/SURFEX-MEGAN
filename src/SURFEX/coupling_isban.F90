...
SUBROUTINE COUPLING_ISBA_n (DTCO, UG, U, USS, NAG, CHI, NCHI, MGN, MSF, DTI, ID, NGB,   &
                            ...
                            PPET_A_COEF, PPEQ_A_COEF, PPET_B_COEF, PPEQ_B_COEF, HTEST, T24, P_PPFD, PPFD24, ICOUNT    )  
!     ###############################################################################
!
...
REAL, DIMENSION(KI), INTENT(OUT) :: P_PPFD     ! PAR                            (umol/m2.s)
!
...
REAL, DIMENSION(KI), INTENT(IN) :: T24    ! average temperature over the past 24h (K)
REAL, DIMENSION(KI), INTENT(IN) :: PPFD24 ! average PAR over the past 24h         (umol/m2.s)
...
REAL, DIMENSION(KI)     :: PGAMMA_SM, PGAMMA_AGE, PACTIVITY_FACTOR, PGAMMA_CO2, P_CO2
!
INTEGER                 ::ISPINEND
!
! Patch outputs:
...
REAL, DIMENSION(KI,IO%NPATCH) :: ZPPFD_TILE, ZGAMMA_SM_TILE, ZGAMMA_AGE_TILE    
REAL, DIMENSION(KI,IO%NPATCH) :: ZACTIVITY_FACTOR_TILE, ZGAMMA_CO2_TILE, ZP_CO2_ppm_TILE
...
!  
 ...
 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZGAMMA_SM_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, PGAMMA_SM   )  

 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZGAMMA_AGE_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, PGAMMA_AGE   )

 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZACTIVITY_FACTOR_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, PACTIVITY_FACTOR   )

 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZGAMMA_CO2_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, PGAMMA_CO2   ) 
                   
 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZP_CO2_ppm_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, P_CO2   )
                   

 CALL AVERAGE_FLUX(S%XPATCH, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, &
                   ZSFCO2_TILE, ZSFU_TILE, ZSFV_TILE, ZPPFD_TILE, PSFTH, PSFTQ,&
                   PSFTS, PSFCO2, PSFU, PSFV, P_PPFD   )  
...
IF (CHI%SVI%NBEQ>0 .AND. CHI%LCH_BIO_FLUX) THEN
  IF (TRIM(CHI%CPARAMBVOC) == 'MEGAN') THEN
         DO I=1,SIZE(MGN%XBIOFLX,1)
            IF ((S%XPATCH(I,1) + S%XPATCH(I,2) + S%XPATCH(I,3)) .LT. 1.) THEN
                 MGN%XBIOFLX(I) = PSFTS(I,MGN%NBIO)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XT24(I) = T24(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XPPFD24(I) = PPFD24(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XPPFD(I) = P_PPFD(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XGAMMA_SM(I) = PGAMMA_SM(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XGAMMA_AGE(I) = PGAMMA_AGE(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XGAMMA_CO2(I) = PGAMMA_CO2(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XPCO2(I) = P_CO2(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
                 MGN%XACTIVITY_FACTOR(I) = PACTIVITY_FACTOR(I)/(1. - S%XPATCH(I,1) - S%XPATCH(I,2) - S%XPATCH(I,3))
            ELSE
                 MGN%XBIOFLX(I) = PSFTS(I,MGN%NBIO)
            ENDIF
         ENDDO
  ENDIF
ENDIF
...
REAL, DIMENSION(PK%NSIZE_P) :: ZP_CO2_ppm     ! CO2 concentration in the air          (ppm) 
...
REAL, DIMENSION(PK%NSIZE_P) :: ZP_PPFD, ZP_GAMMA_SM, ZP_GAMMA_AGE, ZP_ACTIVITY_FACTOR, ZP_GAMMA_CO2               
...
!
 CALL ISBA(IO, KK, PK, PEK, GK, AGK, DK, DEK, DMK,                                            &
           ...
           ZIRRIG_GR, ZP_RNSHADE, ZP_RNSUNLIT )
!
ZP_TRAD = DK%XTSRAD
DK%XLE  = PEK%XLE
!
...
!UPG*PT
 ...
 WHERE (ZP_GLOBAL_SW > 2000.) ! non physical values
  ZP_GLOBAL_SW = 0.
 END WHERE

!convert from kg/kg to ppm
 ZP_CO2_ppm = ZP_CO2*ZP_RHOA*1000000/1.8

 CALL COUPLING_MEGAN_n(MGN, CHI, GK, PEK,KK, &
                       KYEAR, KMONTH, KDAY, PTIME, IO%LTR_ML, &
                       IP_SLTYP, ZP_PFT, ZP_EF, &
                       ZP_TA, ZP_GLOBAL_SW, ZP_TRAD, ZP_RNSUNLIT, ZP_RNSHADE, &
                       ZP_WIND, ZP_PA, ZP_QA, ZP_SFTS, ZP_TEMP, ZP_PPFD, ZP_PAR, &
                       ICOUNT, ZP_GAMMA_SM, ZP_GAMMA_AGE, ZP_ACTIVITY_FACTOR, ZP_CO2_ppm, ZP_GAMMA_CO2)
 ENDIF
ENDIF
! --------------------------------------------------------------------------------------
...
IF (IO%NPATCH==1) THEN
   ...
   ZPPFD_TILE      (:,JP)  = ZP_PPFD      (:)
   ZGAMMA_SM_TILE (:,JP)   = ZP_GAMMA_SM  (:)   
   ZGAMMA_AGE_TILE (:,JP)  = ZP_GAMMA_AGE (:)
   ZGAMMA_CO2_TILE(:,JP)   = ZP_GAMMA_CO2 (:)
   ZP_CO2_ppm_TILE(:,JP)   = ZP_CO2_ppm   (:)
   ZACTIVITY_FACTOR_TILE (:,JP)   = ZP_ACTIVITY_FACTOR  (:)
...
!cdir unroll=8
 DO JJ=1,PK%NSIZE_P
   ...
   IF (PEK%XVEG(JJ) > 0.) THEN 
      ZPPFD_TILE      (JI,JP)   = ZP_PPFD       (JJ)
      ZGAMMA_SM_TILE  (JI,JP)   = ZP_GAMMA_SM   (JJ)
      ZGAMMA_AGE_TILE (JI,JP)   = ZP_GAMMA_AGE  (JJ)
      ZGAMMA_CO2_TILE (JI,JP)   = ZP_GAMMA_CO2  (JJ)
      ZP_CO2_ppm_TILE (JI,JP)   = ZP_CO2_ppm    (JJ)
      ZACTIVITY_FACTOR_TILE (JI,JP)   = ZP_ACTIVITY_FACTOR  (JJ)
   ELSE 
      ZPPFD_TILE(JI,JP) = XUNDEF
      ZGAMMA_SM_TILE(JI,JP)  = XUNDEF
      ZGAMMA_AGE_TILE(JI,JP) = XUNDEF
      ZGAMMA_CO2_TILE(JI,JP) = XUNDEF
      ZP_CO2_ppm_TILE (JI,JP)      = XUNDEF
      ZACTIVITY_FACTOR_TILE(JI,JP) = XUNDEF
   END IF
 ENDDO
!
!cdir nodep
