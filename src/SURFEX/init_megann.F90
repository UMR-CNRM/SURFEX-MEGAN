...
INTEGER:: IP_TRBE, IP_TRBD, IP_TEBE, IP_TEBD, IP_TENE, &
          IP_BOBD, IP_BONE, IP_BOND, IP_SHRB, IP_BOGR
!
...
ALLOCATE(MGN%XPPFD(SIZE(K%XCLAY,1)))
ALLOCATE(MGN%XGAMMA_SM(SIZE(K%XCLAY,1)))
ALLOCATE(MGN%XGAMMA_AGE(SIZE(K%XCLAY,1)))
ALLOCATE(MGN%XACTIVITY_FACTOR(SIZE(K%XCLAY,1)))
ALLOCATE(MGN%XGAMMA_CO2(SIZE(K%XCLAY,1)))
ALLOCATE(MGN%XPCO2(SIZE(K%XCLAY,1)))

MGN%XBIOFLX(:) = 0.
MGN%XGAMMA_SM(:) = 0.
MGN%XGAMMA_AGE(:) = 0.
MGN%XACTIVITY_FACTOR(:) = 0.
MGN%XGAMMA_CO2(:) = 0.
MGN%XPCO2(:) = 0.
...
DO JSV=1,SIZE(HSV)
  ...
  !print*, 'JSV', JSV, TRIM(HSV(JSV)), MGN%CMECHANISM
END DO
!
...
IP_BOGR = VEGTYPE_TO_PATCH(NVT_BOGR, IO%NPATCH)
!
...
! -------------------------------------
!utilisation de la classe NVT_GRAS + NVT_PARK
...
!
!15 Wheat (winter crops)
! -------------------------------------  !!(Ordre de 15 et 16 inversé: voir PFT_MGN.EXT(megan))
! utilisation de la classe NVT_C3
MGN%XPFT(15,:) = S%XVEGTYPE(:,NVT_C3)

!16 Corn (summer crops)
! -------------------------------------
! utilisation de la classe NVT_C4
MGN%XPFT(16,:) = S%XVEGTYPE(:,NVT_C4)
!
