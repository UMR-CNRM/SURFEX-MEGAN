...
SUBROUTINE COUPLING_SURF_ATM_n (YSC, HPROGRAM, HCOUPLING, PTIMEC, PTSTEP, KYEAR, KMONTH,  &
                                ...
                                PPET_B_COEF, PPEQ_B_COEF, HTEST, T24, P_PPFD, PPFD24, ICOUNT         )
!     #################################################################################
!
...
REAL, DIMENSION(KI), INTENT(OUT) :: P_PPFD      ! PAR over nature tile                  (umol/m2.s)
!
...
REAL, DIMENSION(KI,NTILESFC) :: ZSFV_TILE, DUMP_ARRG      ! meridian momentum flux
...
REAL, DIMENSION(KI,NTILESFC) :: ZPPFD_TILE    ! PAR 
!
...
!
 CALL AVERAGE_FLUX(ZFRAC_TILE, ZSFTH_TILE, ZSFTQ_TILE, ZSFTS_TILE, ZSFCO2_TILE, &
                   ZSFU_TILE, ZSFV_TILE, ZPPFD_TILE, PSFTH, PSFTQ, PSFTS, PSFCO2, PSFU, PSFV, P_PPFD )

DO I=1,SIZE(P_PPFD,1)
   IF ((ZFRAC_TILE(I,1) + ZFRAC_TILE(I,2) + ZFRAC_TILE(I,4)) .LT. 1.) THEN
      P_PPFD(I) = P_PPFD(I)/(1. - ZFRAC_TILE(I,1) - ZFRAC_TILE(I,2) - ZFRAC_TILE(I,4))
   ENDIF
ENDDO
...
REAL, DIMENSION(KSIZE) :: ZP_PPFD     ! PAR   
!
...
REAL, DIMENSION(KSIZE) :: ZP_T      !average temperature over the past 24h
REAL, DIMENSION(KSIZE) :: ZP_PAR       !average PAR over the past 24h
...
ELSEIF (KTILE==3) THEN
  ...
  CALL COUPLING_NATURE_n(YSC%DTCO, YSC%UG, YSC%U, YSC%USS, YSC%IM, YSC%DTZ, YSC%DLO, YSC%DL, &
                         ...
                         ZP_PEQ_B_COEF, 'OK', ZP_T, ZP_PPFD, ZP_PAR, ICOUNT       )
  !
ELSEIF (KTILE==4) THEN
  !
...
!cdir unroll=8
  ...
  ENDDO
ENDDO
!Récupérer la sortie de PPFD
IF (KTILE == 3) THEN 
    DO JJ=1,KSIZE   
       ZPPFD_TILE      (KMASK(JJ),KTILE)  = ZP_PPFD     (JJ)
    ENDDO  
END IF 
!
