...
SUBROUTINE EMPROC(KTIME, KDATE, PPFD24, T24, PDI, PRECADJ, &
                  ...
                  PFTF,  OSOIL, PCFNO, PCFNOG, PCFSPEC, PPPFD, PWWILT, PGAMMA_SM, PGAMMA_AGE, PACTIVITY_FACTOR, PCO2, PGAMMA_CO2)
...
REAL, DIMENSION(:), INTENT(INOUT) :: T24, PPFD24  !I DAILY TEMPERATURE (K)
...
REAL, DIMENSION(:), INTENT(IN)    :: PWWILT   !I WILTING POINT (M3/M3)
...
REAL, DIMENSION(:),INTENT(INOUT) :: PPPFD  !! alpha p, alpha t, PPPFD
REAL, DIMENSION(:),INTENT(OUT) :: PGAMMA_AGE, PGAMMA_CO2, PGAMMA_SM, PACTIVITY_FACTOR
...
REAL, DIMENSION(SIZE(PSOILM)) :: ZGAM_CO2  ! CO2 INHIBITION FACTOR
...
REAL, DIMENSION(SIZE(PSOILM)) :: ZPPFD, ZALPHA_P
REAL, DIMENSION(SIZE(PSOILM)) :: HLAIC, HLAIP, HPPFD, HTEMP, HT24, HQV, HWIND, HSOILM, HPPFD24

!!REAL, DIMENSION(SIZE(PSOILM), 5) :: ZPPFD, ZPPFD_AVR !! 
REAL, DIMENSION(SIZE(PSOILM)) :: ZPPFD_AVR, ZALPHA_P_AVR !!
...
DO JS = 1, N_MGN_SPC
  ...
  CALL GAMMA_LAI(HLAIC, ZGAM_LHT)
 
  CALL GAMMA_A(KDATE, KTIME, NTSTLEN, YVNAME3D(JS), HT24, HLAIP, HLAIC, ZGAM_AGE)

  CALL GAMMA_CO2(PCO2, ZGAM_CO2)
  
  !CALL GAMMA_S(PSOILM, PWWILT, ZGAM_SMT)
 ...
 ENDIF
  ...
  DO I_PFT = 1,N_MGN_PFT  !CANOPY TYPES
    ...
    CALL GAMME_CE(KDATE, KTIME, XCANOPYCHAR, I_PFT, YVNAME3D(JS), &
                  HPPFD24, HPPFD24, HT24, HT24, PDI,          &
                  HPPFD, PLAT, PLONG, HTEMP, HWIND, HQV, HLAIC,    &
                  PRES,  ZGAMMA_TD, ZGAMMA_TI, ZPPFD, ZALPHA_P)
    ...
    ZPPFD_AVR(:) = ZPPFD_AVR(:) + PFTF(I_PFT,:) * ZPPFD(:)      !!ligne ajoutée
    ZALPHA_P_AVR(:) = ZALPHA_P_AVR(:) + PFTF(I_PFT,:) * ZALPHA_P(:)      !!ligne ajoutée
...
!...  CALCULATE EMISSION
  ZER(:) = ZGAM_AGE * ZGAM_SMT * ZRHO * ZGAM_CO2 *((1.-ZLDF) * ZGAM_TLI * ZGAM_LHT + ZLDF * ZGAM_TLD)
  WHERE( ZER(:).GT.0. ) 
    PCFSPEC(JS,:) = ZER(:)
  ELSEWHERE
    PCFSPEC(JS,:) = 0.0
  ...
  IF (JS == 1) THEN
    PACTIVITY_FACTOR(:) = PCFSPEC(JS,:)
    print*, PACTIVITY_FACTOR(:)
  ENDIF
