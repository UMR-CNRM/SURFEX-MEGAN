...
SUBROUTINE GAMME_CE(KDATE, KTIME, PCANOPYCHAR, KCANTYPE, HSPCNAME, &
                    ...
                    PLAI, PRES, PEA1CANOPY, PEATICANOPY, PPFD, ALPHA_P)   !!
!
IMPLICIT NONE
! INPUT
...
REAL, DIMENSION(:), INTENT(OUT) :: PPFD, ALPHA_P !!
!
! LOCAL VARIABLES
...
! PSTD = 50 FOR SHADE LEAVES
  ...
  ZEA1LAYER(:,JI) = EA1T99(HSPCNAME   , PT24       , PT240       , ZSUNLEAFTK  (:,JI)) * &
                    ...
                    EA1P99(XPSTD_SUN  , PPFD24 , PPFD24 , ZSUNPPFD    (:,JI)) * ZSUNFRAC(:,JI) +  &
                    EA1T99(HSPCNAME   , PT24       , PT240       , ZSHADELEAFTK(:,JI)) * &
                    !EA1P99(XPSTD_SHADE, PPFD24*0.16, PPFD240*0.16, ZSHADEPPFD  (:,JI) ) * (1.-ZSUNFRAC(:,JI))
                    EA1P99(XPSTD_SHADE, PPFD24*0.25, PPFD24*0.25, ZSHADEPPFD  (:,JI) ) * (1.-ZSUNFRAC(:,JI))
...
!  ZEA1TCANOPY(JJ) = SUM(ZEA1TLAYER(JJ,:) * ZVPSLWWT(JJ,:) * ZVPGAUSWT(:) )
  ...
  PPFD(JJ) = SUM(Z_PPFD(JJ,:) * ZVPSLWWT(JJ,:) * ZVPGAUSWT(:) )
  ALPHA_P(JJ) = SUM(Z_ALPHAP (JJ,:) * ZVPSLWWT(JJ,:) * ZVPGAUSWT(:) ) !!
