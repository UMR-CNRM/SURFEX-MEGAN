...
REAL, DIMENSION(:,:), INTENT(IN) :: PPFD_TILE  ! meridian momentum flux (pa)
...
REAL, DIMENSION(:),   INTENT(OUT):: P_PPFD       ! meridian momentum flux (pa)
!
...
DO JTILE = 1, SIZE(PSFTH_TILE,2)
   DO JJ =1, SIZE(PPFD_TILE,1)
      IF (PPFD_TILE(JJ,JTILE) /= XUNDEF) THEN 
         P_PPFD(JJ)  = P_PPFD(JJ) + PFRAC_TILE(JJ,JTILE) * PPFD_TILE(JJ,JTILE)
      ENDIF
   END DO
END DO
